...
description: 연속 배치 기법은 입력 토큰을 길이와 자원 사용량에 따라 동적으로 묶어 처리하여 GPU 활용 효율을 극대화하고, 배치 간 레이턴시를 최소화합니다.
status: draft
generated:
...
## ⚙️ 핵심 요소
- `continuous_batching` – 배치 결합 로직 및 토큰 길이 기반 동적 병합
- `max_batch_size` – 하드웨어 제한에 따른 최대 배치 크기 (예: 2048 토큰)
- `length_aware_scheduler` – 토큰 길이와 연산량을 고려해 배치를 스케줄링하는 스케줄러

## 🔗 선행 관계
- **PREREQUISITES**: `kv_cache` (기본 KV 캐시 엔진)
