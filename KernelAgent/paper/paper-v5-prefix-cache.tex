...
\title{cuTile 기반 Windows-native LLM 추론 엔진 이식, Hybrid Prefill Dispatch, 그리고 고정 컨텍스트 Agent Workload에서의 Prefix KV Cache 실증}
\engtitle{Windows-Native LLM Inference Engine Migration with cuTile, Hybrid Prefill Dispatch, and Prefix KV-Cache Evaluation for Fixed-Context Agent Workloads}
\author{}
\engauthor{
\vspace{6mm}
김태원\\ (Taewon Kim)\\
...
\item \textbf{Hybrid prefill dispatch와 CUDA Graph negative result}: cuTile KV-cache store repair 이후 eager-hybrid cuTile 경로가 557.66 tok/s로 초기 cuTile 평균 대비 20.4\% 개선됨을 보이고, CUDA Graph decode replay가 기능적으로 가능하더라도 현재 kernel granularity와 graph bucket replay 조건에서는 약 393 tok/s로 성능을 저하시킬 수 있음을 실증하였다.
...
\subsection{cuTile Hybrid Prefill Dispatch와 CUDA Graph Replay 결과}
초기 Windows-native cuTile backend는 평균 463.35 tok/s를 기록하였다. 이후 두 가지 수정을 적용하였다. 첫째, decode graph capture 중 PyTorch advanced indexing으로 KV cache를 갱신하던 경로를 cuTile KV-store kernel로 대체하였다. 둘째, prefill wrapper를 workload에 따라 선택하는 hybrid prefill dispatch를 도입하였다. 이 정책은 multi-sequence non-prefix prefill에서는 batched padded wrapper를 사용하여 host/kernel launch 수를 줄이고, single-sequence 또는 prefix-cache prefill에서는 direct/paged wrapper를 사용하여 불필요한 tensor materialization을 피한다.

\begin{table}[h]
\centering
\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{tabular}{|p{0.32\linewidth}|p{0.14\linewidth}|p{0.20\linewidth}|p{0.18\linewidth}|p{0.12\linewidth}|}
\hline
\textbf{cuTile Mode} & \textbf{Graph} & \textbf{Prefill} & \textbf{Time} & \textbf{Throughput} \\ \hline
Initial cuTile mean & off & eager baseline & 289.16 s & 463.35 tok/s \\ \hline
Hybrid eager & off & hybrid & 240.23 s & 557.66 tok/s \\ \hline
Hybrid graph & on & hybrid & 340.60 s & 393.32 tok/s \\ \hline
Padded graph & on & padded & 341.02 s & 392.84 tok/s \\ \hline
Direct graph & on & direct & stopped early & 95--101 tok/s progress \\ \hline
\end{tabular}
\caption{cuTile KV-store repair 이후 prefill dispatch 및 CUDA Graph replay 비교}
\end{table}

Eager-hybrid cuTile 경로는 초기 cuTile 평균 463.35 tok/s에서 557.66 tok/s로 20.4\% 개선되었다. 그러나 CUDA Graph replay는 capture 자체에는 성공했음에도 throughput을 약 393 tok/s 수준으로 낮추었다. 이는 CUDA Graph가 보편적으로 유리한 최적화가 아니라, graph bucket replay의 overcompute, kernel granularity, paged decode memory access pattern이 맞물릴 때 오히려 E2E throughput을 악화시킬 수 있음을 보여준다. 따라서 본 논문에서는 hybrid eager를 현재의 paper-positive cuTile 결과로 사용하고, CUDA Graph replay는 성공적으로 수리된 negative result로 해석한다.
...
micro-vLLM은 production vLLM보다 빠르지 않다. 그러나 KTCP 관점에서 본 연구의 실용적 가치는 Windows-native 환경에서 LLM serving loop를 직접 실행하고 수정할 수 있으며, MatMul부터 micro-vLLM까지 단계적 학습 경로를 제공하고, 성공한 최적화(prefix cache, hybrid eager dispatch)와 실패하거나 조건부로 악화된 최적화(shape padding, CUDA Graph replay)를 모두 재현할 수 있다는 점에 있다. 특히 CUDA Graph는 capture 성공 여부와 performance gain이 별개라는 점을 보여주는 교육적으로 중요한 사례이다.
...
\section{한계}
본 연구에는 다음 한계가 있다. 첫째, 단일 RTX 5070 target PC 결과이므로 다른 GPU나 datacenter 환경으로 일반화하기 어렵다. 둘째, Windows-native cuTile backend는 WSL2 FlashAttention보다 크게 느리므로 production performance claim으로 해석하면 안 된다. 셋째, Prefix cache benchmark는 64-token generation으로 고정되어 decode-dominated 상황에서 E2E throughput 개선을 보이지 않았다. 넷째, Prefix cache 효과는 exact token-prefix stability에 의존한다. 다섯째, Green Contexts 결과는 current sequential Python engine loop에서는 mixed result로 나타났다. 여섯째, CUDA Graph replay는 capture repair 이후에도 현재 cuTile decode kernel과 graph bucket 조건에서 performance-negative로 나타났으므로, 최종 성능 경로가 아니라 negative systems result로 해석해야 한다.

\section{결론}
본 논문은 CUDA Python 1.0과 cuTile-style workflow를 이용해 Windows-native LLM inference engine artifact인 micro-vLLM을 구현하고, 이를 교육 가능한 systems artifact로 해석하였다. WSL2 FlashAttention은 초기 Windows cuTile backend보다 평균 4.62배 높은 throughput을 보였으므로, micro-vLLM의 기여는 production serving 성능 우위가 아니라 Windows-native migration path, inspectable implementation, reproducible bottleneck analysis에 있다. 다만 cuTile KV-store repair와 hybrid prefill dispatch 이후 eager-hybrid cuTile 경로는 557.66 tok/s를 기록하여 초기 cuTile 평균 대비 20.4\% 개선되었고, CUDA Graph replay는 기능적으로 가능하나 현재 조건에서는 약 393 tok/s로 성능을 낮추는 negative result로 확인되었다.
